...
\Def{}{}{Let $D_1$ and $D_2$ be two distributions. An algorithm $A:\{O,1\}^n\rightarrow\{O,1\}$ with support $\{O,1\}^n$ is distinguishable if $Adv_A(D_1,D_2) := |\Pr_{x\hookrightarrow D_1}(A(x)=1)-\Pr_{x\hookrightarrow D_2}(A(x)=1)|$ is non-negligible.}

$G(\{O,1\}^s)$ is a tiny subset of $\{O,1\}^n$ but we want it to look uniform.

\Def{Indistinguishability from uniform}{}{$D_1$ and $D_2$ are indistinguishable if for any PPT $A:\{0,1\}^n\rightarrow\{0,1\}$ $Adv_A(D_1,D_2)$ is negligible.}

\Def{}{}{A PRG $G$ is pseudo-random is $D_1:=\{G(k)|k\hookrightarrow U(\{0,1\}^s)\}$ and $D_2:=\{x|x\hookrightarrow U(\{0,1\}^n)\}$ are indistinguishable} 

...
\Def{CCA security}{def:CCAsecure}{A symmetric encryption scheme $(Keygen,Enc,Dec)$ is CCA secure if no PPT algorithm $\A$ adversary can win the following gamie more than negligible advantage:
...
\draw	[->]	(4.5,1.5)	--node[above]	{$c_i$}		(1.5,1.5);
\draw	[->]	(1.5,0.5)	--node[above]	{$Dec(k,c_i)$}	(4.5,0.5);
